...
class STANet(nn.Module):
    def __init__(self, feature_extractor, target_model, target_model_segm, decoder, target_model_input_layer, decoder_input_layers,
                 label_encoder=None, bbox_encoder=None, segm_encoder = None):
        super().__init__()
        ...
        self.target_model_segm = target_model_segm          # Target model and the few-shot learner
    ...
    def forward(self, train_imgs, train_bbox):
        ...
        # pred_all = pred_all.detach()
        # train_bbox_enc = train_bbox_enc.detach()
    ...
    # def segment_target(self, target_filter, test_feat_clf, test_feat, segm):
    #     # Classification features
    #     assert target_filter.dim() == 5     # seq, filters, ch, h, w
    #     test_feat_clf = test_feat_clf.view(-1, 1, *test_feat_clf.shape[-3:])

    #     if segm == False:
    #         target_scores = self.target_model.classify(target_filter, test_feat_clf)
    #     else:
    #         target_scores = self.target_model_segm.classify(target_filter, test_feat_clf)
    #     mask_pred, decoder_feat = self.decoder(target_scores, test_feat,
    #                                            (test_feat_clf.shape[-2]*16, test_feat_clf.shape[-1]*16),
...
def steepest_descent_resnet50(filter_size=1, num_filters=1, optim_iter=3, optim_init_reg=0.01,
    ...
    net = STANet(feature_extractor=backbone_net, target_model=target_model, target_model_segm=target_model_segm, decoder=decoder,
