...
import ltr.models.lwl.sta_net as sta_networks
...
def run(settings):
    ...
    data_processing_train = processing.STAProcessing(search_area_factor=settings.search_area_factor,
    ...
    data_processing_val = processing.STAProcessing(search_area_factor=settings.search_area_factor,
                                                   output_sz=settings.output_sz,
                                                   center_jitter_factor=settings.center_jitter_factor,
                                                   scale_jitter_factor=settings.scale_jitter_factor,
                                                   mode='sequence',
    ...
    dataset_train = sampler.STASampler([ytvos_train, davis_train], [6, 1],
                                       samples_per_epoch=settings.batch_size * 1000, max_gap=100,
                                       num_train_frames=3,
                                       processing=data_processing_train)
    dataset_val = sampler.STASampler([ytvos_val], [1],
                                     samples_per_epoch=settings.batch_size * 500, max_gap=100,
                                     num_train_frames=3,
    ...
    # Network
    net = sta_networks.steepest_descent_resnet50(filter_size=3, num_filters=16, optim_iter=5,
                                                 backbone_pretrained=True,
                                                 out_feature_dim=512,
                                                 frozen_backbone_layers=['conv1', 'bn1', 'layer1', 'layer2', 'layer3',
                                                                         'layer4'],
                                                 label_encoder_dims=(16, 32, 64, 128),
    ...
    actor = segm_actors.STAActor(net=net, objective=objective, loss_weight=loss_weight,
    ...
    optimizer = optim.Adam([{'params': actor.net.target_model.filter_initializer.parameters(), 'lr': 5e-5},
                            ...
                            {'params': actor.net.bbox_encoder.parameters(), 'lr': 2e-4},
                            {'params': actor.net.segm_encoder.parameters(), 'lr': 2e-4},
                            {'params': actor.net.target_model_segm.filter_initializer.parameters(), 'lr': 5e-5},
                            {'params': actor.net.target_model_segm.filter_optimizer.parameters(), 'lr': 1e-4}],
                           lr=2e-4)
