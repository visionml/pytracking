...
import ltr.models.lwtl.lwtl_net as seg_tracker
...
def run(settings):
    ...
    dataset_train = sampler.LWTLSampler([ytvos_train, davis_train], [6, 1],
                                        samples_per_epoch=settings.batch_size * 1000, max_gap=100, num_test_frames=3,
                                        num_train_frames=1,
                                        processing=data_processing_train)
    dataset_val = sampler.LWTLSampler([ytvos_val], [1],
    ...
    # Network
    net = seg_tracker.steepest_descent_resnet50(filter_size=3, num_filters=16, optim_iter=5,
    ...
    base_net = network_loading.load_trained_network(settings.env.workspace_dir,
                                                    'ltr/segm/lwtl_stage1/LWTLNet_ep0070.pth.tar')
    ...
    actor = segm_actors.LWTLActor(net=net, objective=objective, loss_weight=loss_weight,
