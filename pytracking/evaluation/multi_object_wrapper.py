...
class MultiObjectWrapperSTA:
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self.initialized_ids = info['init_object_ids'].copy()
        self.initialized_ids_for_store = info['init_object_ids'].copy()
    ...
    def track(self, image, bbox, info: dict = None) -> dict:
        ...
        for obj_id in self.initialized_ids:
            ...
            out = self.trackers[obj_id].track(image, bbox, info)
        ...
        # Initialize new
        if info.get('init_object_ids', False):
            if info['init_object_ids'] != self.initialized_ids:
                for obj_id in info['init_object_ids']:
                    # if not obj_id in self.trackers:
                    #     self.trackers[obj_id] = self.create_tracker()

                    start_time = time.time()
                    self.trackers[obj_id].track(image, bbox, info)
                    # out = self.trackers[obj_id].initialize(image, init_info_split[obj_id])
                    # if out is None:
                    out = {}
    ...
    def store_seq(self, image, bbox, info):
        ...
        for obj_id in self.initialized_ids_for_store:
            out = self.trackers[obj_id].store_seq(image, bbox, info)
        ...
        if info.get('init_object_ids', False):
            for obj_id in info['init_object_ids']:
                ...
                out = self._set_defaults(out, init_default)
            self.initialized_ids_for_store.extend(info['init_object_ids'])
